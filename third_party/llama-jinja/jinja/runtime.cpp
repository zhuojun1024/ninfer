...
void enable_debug(bool enable) { g_jinja_debug = enable; }
...
value identifier::execute_impl(context& ctx) {
    ...
    if (!it->is_undefined()) {
        if (ctx.is_get_stats) { value_t::stats_t::mark_used(it); }
...
value object_literal::execute_impl(context& ctx) {
    ...
    for (const auto& pair : val) {
        ...
        JJ_DEBUG("Object literal: setting key '%s' with value type %s",
                 key->as_string().str().c_str(), val->type().c_str());
        obj->insert(key, val);
    }
    return obj;
}
...
value binary_expression::execute_impl(context& ctx) {
    ...
    auto workaround_concat_null_with_str = [&](value& res) -> bool {
        ...
        if ((is_left_null && is_right_str) || (is_right_null && is_left_str)) {
            JJ_DEBUG(
                "%s",
                "Workaround: treating null/undefined as empty string for string concatenation");
    ...
    if (is_val<value_undefined>(left_val) || is_val<value_undefined>(right_val)) {
        ...
        if (op.value == "+" || op.value == "~") {
            ...
            if (workaround_concat_null_with_str(res)) { return res; }
        }
    ...
    if (op.value == "*" && ((is_val<value_string>(left_val) && is_val<value_int>(right_val)) ||
        ...
        for (int64_t i = 0; i < repeat; ++i) { res->val_str.append(str); }
        return res;
    }
    ...
    if (is_stmt<identifier>(filter)) {
        ...
        if (!input->is_undefined() && !is_val<value_string>(input) &&
            (filter_id == "capitalize" || filter_id == "lower" || filter_id == "replace" ||
             ...
             filter_id == "wordcount")) {
            JJ_DEBUG("Coercing %s to String for '%s' filter", input->type().c_str(),
                     filter_id.c_str());
        ...
        set_filter_alias(filter_id);
        JJ_DEBUG("Applying filter '%s' with arguments to %s", filter_id.c_str(),
                 input->type().c_str());
        func_args args(ctx);
...
value filter_statement::execute_impl(context& ctx) {
    ...
    JJ_DEBUG("FilterStatement: applying filter to body string of length %zu",
             parts->val_str.length());
...
value test_expression::execute_impl(context& ctx) {
    ...
    JJ_DEBUG("Test expression %s '%s' %s (using function '%s')", operand->type().c_str(),
             test_id.c_str(), negate ? "(negate)" : "", test_name.c_str());
...
value set_statement::execute_impl(context& ctx) {
    ...
    if (is_stmt<identifier>(assignee)) {
        ...
        JJ_DEBUG("Setting global variable '%s' with value type %s", var_name.c_str(),
                 rhs->type().c_str());
        ...
        JJ_DEBUG("Setting object property '%s' with value type %s", prop_name.c_str(),
                 rhs->type().c_str());
...
static inline void bind_parameters(const std::string& name, const statements& this_args,
                                   const func_args& args, context& ctx) {
    const size_t expected_count = this_args.size();
    const size_t input_count    = args.count();

    JJ_DEBUG("Invoking '%s' with %zu input arguments (expected %zu)", name.c_str(), input_count,
             expected_count);
        ...
        if (i < input_count) {
            if (is_stmt<identifier>(this_args[i])) {
                ...
                if (!is_stmt<identifier>(kwarg->key)) {
                    throw std::runtime_error("Keyword argument key must be an identifier in '" +
                                             name + "'");
                }
            ...
            if (is_stmt<keyword_argument_expression>(default_arg)) {
                ...
                std::string param_name = cast_stmt<identifier>(kwarg->key)->val;
                JJ_DEBUG("  Binding parameter '%s' to default argument of type %s",
                         param_name.c_str(), kwarg->val->type().c_str());
...
value macro_statement::execute_impl(context& ctx) {
    ...
    const func_handler func = [this, name](const func_args& args) -> value {
        ...
        JJ_DEBUG("Macro '%s' execution complete, result: %s", name.c_str(),
                 res->val_str.str().c_str());
        return res;
    };
...
value call_statement::execute_impl(context& ctx) {
    ...
    const func_handler func = [this,
                               caller_ctx = std::move(caller_ctx)](const func_args& args) -> value {
...
value member_expression::execute_impl(context& ctx) {
    ...
    if (this->computed) {
        ...
        if (is_stmt<slice_expression>(this->property)) {
            ...
            value start_val = s->start_expr
                                  ? s->start_expr->execute(ctx)
                                  : (step_val->as_int() < 0 ? mk_val<value_int>(arr_size - 1)
                                                            : mk_val<value_int>(0));
            value stop_val  = s->stop_expr ? s->stop_expr->execute(ctx)
                                           : (step_val->as_int() < 0 ? mk_val<value_int>(-1)
                                                                     : mk_val<value_int>(arr_size));

            // translate to function call: obj.slice(start, stop, step)
            JJ_DEBUG("Member expression is a slice: start %s, stop %s, step %s",
                     start_val->as_repr().c_str(), stop_val->as_repr().c_str(),
        ...
        JJ_DEBUG("Member expression, object type %s, static property '%s'", object->type().c_str(),
                 prop.c_str());
    ...
    JJ_DEBUG("Member expression on object type %s, property type %s", object->type().c_str(),
             property->type().c_str());
    ...
    if (is_val<value_undefined>(object)) {
        ...
        if (is_val<value_int>(property)) {
            ...
            auto key = property->as_string().str();
            JJ_DEBUG("Accessing %s built-in '%s'", is_val<value_array>(object) ? "array" : "string",
                     key.c_str());
