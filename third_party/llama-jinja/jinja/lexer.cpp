...
lexer_result lexer::tokenize(const std::string& source) {
    ...
    if (source.empty()) { return {tokens, src}; }
    ...
    auto consume_while = [&](const pred& predicate) -> std::string {
        ...
        while (predicate(src[pos])) {
            ...
            if (src[pos] == '\\') {
                ...
                if (pos >= src.size()) {
                    throw lexer_exception("unexpected end of input after escape character", source,
                                          pos);
                }
                // add escaped char
                char escaped_char = src[pos++];
                if (escape_chars.find(escaped_char) == escape_chars.end()) {
                    throw lexer_exception(std::string("unknown escape character \\") + escaped_char,
                                          source, pos);
                }
    ...
    while (pos < src.size()) {
        ...
        token::type last_token_type = tokens.empty() ? token::close_statement // initial state
                                                     : tokens.back().t;
        if (last_token_type == token::close_statement ||
            last_token_type == token::close_expression || last_token_type == token::comment) {
            ...
            is_lstrip_block =
                src[pos] == '{' && next_pos_is({'{', '%', '#'}) && next_pos_is({'-'}, 2);
            if (is_lstrip_block) {
        ...
        if (src[pos] == '-' && (last_token_type == token::open_expression ||
        ...
        for (const auto& [seq, typ] : ordered_mapping_table) {
            ...
            if (seq == "}}" && curly_bracket_depth > 0) { continue; }
