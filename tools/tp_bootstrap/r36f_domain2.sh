#!/bin/bash
# Confirmation: 4 more samples on the MTP route with the vocab token domain.
set -u
cd /home/zhuojun/prof
AB=/home/zhuojun/prof/ab_domain2.log
: > "$AB"
...
for i in 1 2 3 4; do
  curl -s --max-time 300 http://127.0.0.1:8088/v1/chat/completions -H 'Content-Type: application/json' \
    --data-binary @/home/zhuojun/prof/essay_req.json -o "/home/zhuojun/prof/dom2-$i.json" \
    -w "dom2-$i http=%{http_code} wall=%{time_total}s\n" >> "$AB" 2>&1
done
pkill -9 -f 'build/apps/ninfer-serve' 2>/dev/null || true
python3 - <<'PY' >> "$AB" 2>&1
import json, glob, re, statistics
rows = []
for path in sorted(glob.glob("/home/zhuojun/prof/dom2-*.json")):
    try:
    ...
    rows.append(len(c))
print("CONFIRM n=%d median=%d degenerate(<200)=%d" % (
    len(rows), statistics.median(rows) if rows else -1, sum(1 for x in rows if x < 200)))
PY
echo "DOMAIN2_DONE" >> "$AB"
