...
Plan resolve_plan(std::int32_t input_rows, std::int32_t width, std::int32_t batch) {
    ...
    return {select_q4_a16_launch(kHidden, input_rows, tokens),
...
const char* q4_linear_dynamic_grouped_conv_add_route_name(std::int32_t input_rows,
    ...
    return q4_projection_route_label(resolve_plan(input_rows, width, batch_size).projection);
}
...
void q4_linear_dynamic_grouped_conv_add_dispatch(const Tensor& x, const Weight& weight,
    ...
    q4_dynamic_grouped_conv_add_materialized_launch(plan.projection, x, weight, base, delta,
