...
the repository root. Four PLAN.md snapshots are archived verbatim below under their own
`PLAN.md archive` headers: the 2026-09-21 upstream cherry-pick re-plan, the 2026-09-22
delivery-cleanup rewrite, the 2026-09-24 post-protocol-refactor state, and the 2026-09-25
pre-organization state (r67-r70 draft-quantization campaign, TP-2 transport investigation,
TP-2 orchestration/rendezvous work).
...
# PLAN.md archive (archived 2026-09-25, draft-Q4 official-recipe promotion + TP-2 transport/orchestration work)

Archived verbatim from the working PLAN.md when it was reduced to key information and remaining work, after the DFlash2 draft QKV/codebook q4 campaign r67-r70 (2026-09-24/25) and the official-recipe promotion (r69, 2026-09-25), the TP-2 low-frequency transport-failure investigation (§3.10, 2026-09-25), and the TP-2 orchestration and rendezvous work (§3.11, 2026-09-25). Historical record only: the current state and the remaining work live in `PLAN.md` at the repository root, product behavior and measurements in [tp2-dual-5060ti.md](tp2-dual-5060ti.md).
...
**③ 完成（载荷声明部分）**：`text.h` 在 `set_tp_peer` 之后新增**握手协议总表**——9 条协议逐条列出
载荷类型/形状、传输方式与契约（行块 gather、I32 id 广播、hidden 广播 ×3、candidate 合并、selector
forward/back 打包、vision embedding 广播），并写明判据：**载荷决定传输**——`allreduce` 只有 BF16 安全位型
且只作「零缓冲广播/不相交 gather」，`sendrecv` 才是 id/FP32/必须逐字节不变的载荷的通道。

**并且把声明变成可执行的检查**：`dflash_selector_tp2` 的前向打包先把
`forward_payload = ids+scores+anchors+frontiers` 算出来，打包结束后断言
`packed == forward_payload`（两侧打包/解包顺序不一致或字段增减会立刻抛错）。
**该断言立即抓到一个真实不一致**：打包累加器漏了最后一句 `packed += frontiers.bytes()`
（最后一个 memcpy 之后没有累加，属于"看着对但一旦有人依赖累加器就错"的形态），已补齐。

**批次外的实测收获（自然 give-up 首次以安全方式失败）**：③ 改完后的首次三件套里 `solo-a` 撞到一次
**自然** give-up（`TP-2 allreduce stalled at rendezvous id 4294967296` = 通道 1、base 0，即第二个图的
第一个 collective）。这是本战役**第 3 次自然发生**（前两次见 §3.10：r67 的 illegal address 与 r69 的 503），
而**这一次是干净失败**：进程存活、报设计好的失败消息、看门狗因未开启而无 dump。重跑 **5/5 exit=0**，
digest 与改动前逐位相同 ⇒ 该次是既有偶发，非本次改动引入；同时它是 1b 在自然路径上（不只是注入路径）
把"崩溃"变成"设计好的失败"的现场证据。
...
- (c) **止损（已实施 2026-09-25）**：默认超时 **2000 ms → 10000 ms**
  （`device_pair.cu:511`，env `NINFER_TP2_AR_TIMEOUT_MS` 仍可覆盖，0 关闭）。
  判据与取舍写在代码注释里：10 s 是**最慢合法 collective（~2.7 ms 全宽 prefill 载荷；decode 侧 9–18 µs）的
  ~3700×**；这个界只为打破**永久** desync（任何值都解不开），所以必须远高于健康轮次能产生的一切——
  **秒级停顿是 host/driver 事件（WDDM 调度、换页、驱动争用），不是慢 kernel**。两个代价不对称：
  误触发要赔**一整个请求 + 其 re-prefill**，而放宽只让一次本来就会失败的等得更久，且自然事件率约
  **3/1500 请求**。仍是有界自旋，真 desync 不会无限期卡住服务。
